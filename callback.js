...
function handMade(test, callback) {
  if (typeof test === "string" && typeof callback === "function") {
    ...
    // 0번째 매개변수는 "문자열" 이여야만 하고, 1번째 매개변수는 "함수"이여야만 '참(true)'판정을 낸다.
    console.log(typeof test); // "문자열이 출력"
    console.log(typeof callback); // "function"이라는 문자열이 출력
    // 위의 callback이라는 매개변수의 타임체크 방법을 눈여겨보면 함수호출의 방식인 () 소괄호를
    //별도로 붙여주지 않은 것을 확인한다.
    // 자바스크립트에서 소괄호가 없므녀 '호출(call)'을 하지 않았다는 표시이므로 주의

    console.log(callback(test));
    // test라는 인자는 부모함수인 handMade 함수에서 받아온 인자이다.
  }
}
...
/*
  같은 함수를 두번 호출 했는데, 매개변수로 사용된 함수는 다르다.
  위의 함수는 콜백으로 만드는 의미가 전혀 없긴 하지만, 명확하게 변수를 다시 부르는 형태를 통해 실행되었다.
